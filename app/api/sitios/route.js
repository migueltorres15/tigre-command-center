...
export async function GET() {
  try {
    const db = getDb();
    const sitios = await db.prepare('SELECT * FROM sitios ORDER BY created_at DESC').all();
    const tareas = await db.prepare('SELECT * FROM sitio_tareas ORDER BY created_at ASC').all();
...
export async function POST(request) {
  try {
    ...
    const sitio = await db.prepare('SELECT * FROM sitios WHERE id = ?').get(result.lastInsertRowid);
