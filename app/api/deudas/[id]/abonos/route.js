...
export async function GET(request, { params }) {
  const db = getDb();
  const { id } = params;
  const abonos = db.prepare('SELECT * FROM abonos WHERE deuda_id = ? ORDER BY fecha DESC').all(id);
...
export async function POST(request, { params }) {
  try {
    ...
    db.prepare('INSERT INTO abonos (deuda_id, monto, fecha, nota) VALUES (?, ?, ?, ?)').run(id, monto, fechaStr, nota);

    // Update acumulado en deudas
    const nuevoAbonado = (deuda.abonado || 0) + monto;
    const pagada = nuevoAbonado >= deuda.monto ? 1 : 0;
    db.prepare('UPDATE deudas SET abonado = ?, pagada = ? WHERE id = ?').run(nuevoAbonado, pagada, id);
...
export async function DELETE(request, { params }) {
  try {
    ...
    const abono = db.prepare('SELECT * FROM abonos WHERE id = ? AND deuda_id = ?').get(abonoId, id);
