...
export async function GET() {
  try {
    const db = getDb();
    const sitios = db.prepare('SELECT * FROM sitios_usa ORDER BY created_at DESC').all();
    const tareas = db.prepare('SELECT * FROM sitios_usa_tareas ORDER BY created_at ASC').all();
...
export async function POST(request) {
  try {
    ...
    const sitio = db.prepare('SELECT * FROM sitios_usa WHERE id = ?').get(result.lastInsertRowid);
