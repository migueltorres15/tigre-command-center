...
export async function PUT(request, { params }) {
  try {
    ...
    const existing = await db.prepare('SELECT * FROM sesiones WHERE id = ?').get(id);
    if (!existing) {
    ...
    const updated = await db.prepare('SELECT * FROM sesiones WHERE id = ?').get(id);
...
export async function DELETE(request, { params }) {
  try {
    ...
    await db.prepare('DELETE FROM sesiones WHERE id = ?').run(id);
